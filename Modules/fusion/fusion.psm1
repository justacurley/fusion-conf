...
function New-SampleHealthEntries {
    ...
    Valid values: "mood", "vitals", "medication", "activity", "pain", "weight", "sleep"
    ...
    .EXAMPLE
    $samples = New-SampleHealthEntries -EntryTypes @("pain", "medication") -Count 10
    ...
    param(
        ...
        [Parameter(Mandatory = $false)]
        [ValidateSet("mood", "vitals", "medication", "activity", "pain", "weight", "sleep")]
        [string[]]$EntryTypes,
    ...
    try {
        ...
        if ($Count -eq 0) {
            ...
            return if ($OutputFormat -eq "Array") { @() } else { @{} }
        }

        $entries = @{}
        $entryTypes = @("mood", "pain", "vitals", "medication", "activity", "sleep", "weight")
        ...
        if ($PSBoundParameters.ContainsKey('MedicationCount')) { $typeCounts['medication'] = $MedicationCount; $totalTypeCount += $MedicationCount; $typeCountsProvided = $true }
        if ($PSBoundParameters.ContainsKey('ActivityCount')) { $typeCounts['activity'] = $ActivityCount; $totalTypeCount += $ActivityCount; $typeCountsProvided = $true }
                ...
                switch ($type) {
                    ...
                    "pain" {
                        $data.pain = @{
                            ...
                            severity = Get-Random -Minimum 1 -Maximum 10  # Schema v2.0: 1-10 scale
                            note = "Sample pain entry"  # Schema v2.0: 'note' not 'pain_note'
                        }
                    }
                    "vitals" {
                    ...
                    "medication" {
                        ...
                        $data.medication = @{
                            medication_name = $med  # Schema v2.0: 'medication_name' not 'name'
                            dosage = $dosages | Get-Random
                            # Schema v2.0: No 'time_taken' field
                        }
                    }
                    "activity" {
                        $data.activity = @{
                            activity_name = $activities | Get-Random  # Schema v2.0: 'activity_name' not 'type'
                            duration_minutes = Get-Random -Minimum 5 -Maximum 120
                            note = "Sample activity"  # Schema v2.0: 'note' not 'activity_note'
                            # Schema v2.0: No 'intensity' field
                        }
                    }
                    "weight" {
